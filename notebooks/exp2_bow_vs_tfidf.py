        ...
        for vec_name, vectorizer in vectorizers.items():
            with mlflow.start_run(run_name=f"{algo_name} with {vec_name}", nested=True) as child_run:
                ...
                if algo_name == 'LogisticRegression':
                    mlflow.log_param("C", model1.C)
                elif algo_name == 'MultinomialNB':
                    mlflow.log_param("alpha", model1.alpha)
                elif algo_name == 'XGBoost':
                    mlflow.log_param("n_estimators", model1.n_estimators)
                    mlflow.log_param("learning_rate", model1.learning_rate)
                ...
                # Log model
                model_path = f"models/{algo_name}_{vec_name}"
                mlflow.sklearn.save_model(model1, model_path)
